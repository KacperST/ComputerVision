...
# Ustaw ścieżki
curr_dir = os.getcwd()
test_rgb = join(curr_dir,"test_rgb")  # Ścieżka do foldeu test_rgb
test_thermal = join(curr_dir,"test_thermal")  # Ścieżka do foldeu test_thermal
###################################
...
net_rgb = None
if FUSION == "EARLY":
    net_fus = cv2.dnn.readNet(join(curr_dir,'yolov3_training_last_f.weights'), join(curr_dir,'yolov3_testing_f.cfg'))
if FUSION == "LATE":
...
for i in range(200, 300): # można zmieniać range do 518
    ...
    if FUSION == "EARLY":
        ...
        new_fus = np.zeros((img_rgb.shape[0], img_rgb.shape[1], 3))
        new_fus[:, :, :2] = img_rgb[:, :, :2]
    ...
    if FUSION == "LATE":
        ...
        boxes = [[int(sum([Rect1[a][i] for a in elem]) / len(elem)) for i in range(4)] for elem in paired_boxes]
